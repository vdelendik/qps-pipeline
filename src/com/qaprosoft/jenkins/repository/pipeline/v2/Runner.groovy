...
*/	}
	...
	protected void build(params, vars) {
		context.stage('Run Test Suite') {
			...
			DEFAULT_BASE_MAVEN_GOALS += " -Dscreen_record_host=https://${QPS_HOST}/video/%s.mp4 -Dvnc_protocol=wss -Dvnc_host=${SELENIUM_HOST} -Dvnc_port=443 "
	...
	protected String chooseNode(params) {
		...
		def customCapabilities = params.get("custom_capabilities")
		if (!isParamEmpty(customCapabilities) && customCapabilities.contains("browserstack")) {
